...
<div class="container">
            ...
            <div class="col-md-12">
                ...
                <div class="col-lg-8">
                    <table class="table">
                    	...
                    	<tr>
                    		<td>Dạy lớp:</td>
                    		<td class="text-right"><strong><?php echo implode(', ',explode(',', $giasu->gs_lop)); ?></strong></td>
                    	</tr>
                    	<tr>
                    		<td>Dạy môn:</td>
                    		<td class="text-right"><strong><?php echo implode(', ',explode(',', $giasu->gs_mon)); ?></strong></td>
                    	</tr>
                    	<tr>
                    		<td>Dạy các buổi:</td>
                ...
                <tbody id="tbody_ds_sp">
                    ...
                    foreach ($result as $result){
                        ...
                        <tr> 
                            ...
                            <td><button type="button" class="btn btn-default" data-toggle="modal" data-target="#myModal_cmt"><i class=" glyphicon glyphicon-comment"></i></button></td>
                        </tr> 
                            <!-- Modal detail-->
                            <div class="modal fade" id="myModal_cmt" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    ...
    <div class="col-md-4">
    	<div class="panel panel-primary">
        <div class="panel-heading">Gia sư <?php echo implode('/',explode(',', $giasu->gs_mon)); ?></div>
        
            <?php
            //var_dump(count(explode(',',$giasu->gs_mon)));
            $sql="SELECT * FROM giasu WHERE (";
            $num=count(explode(',',$giasu->gs_mon)); 
            if($num>1){
                $gs_mon = explode(',',$giasu->gs_mon);
                $i=0;
                foreach ($gs_mon as $gs_mon) {
                    $i++;
                    $sql .= " gs_mon LIKE '%".$gs_mon."%'";
                    if($i<$num)
                        $sql .= " OR";
                    else
                        $sql .= ") AND gs_tk_id!=$giasu->gs_tk_id AND gs_trangthai=1";
                }
            }
            else
                $sql="SELECT * FROM giasu WHERE gs_tk_id!=$giasu->gs_tk_id AND gs_mon like '%".$giasu->gs_mon."%' AND gs_trangthai=1";
            //echo $sql; var_dump($this->db->query($sql)->result());
            $gs_khac=$this->db->query($sql)->result();

            $i=0;
            if(empty($gs_khac)) echo "<div class=panel-body>Chưa có gia sư khác dạy $giasu->gs_mon</div>";
            foreach ($gs_khac as $gs_khac) { 
                $i++; 
                if($i==6)
                    break;?>
                <div class="panel-body">
                    <div class="col-md-4">
                        <a href="<?php echo base_url(); ?>giasu/detail/<?php echo $gs_khac->gs_tk_id; ?>">
                        <img src="<?php echo base_url().$gs_khac->gs_hinhanh;?>" class="img-responsive">
                        </a>
                    </div>
                    <div class="col-md-8">
                        <a href="<?php echo base_url(); ?>giasu/detail/<?php echo $gs_khac->gs_tk_id; ?>"><b><?php echo $gs_khac->gs_hoten; ?></a></b> <br/>
                        <?php echo "Trình độ: ".$gs_khac->gs_trinhdo; ?> <br/>
                        <?php echo "Giá tiền: ".$gs_khac->gs_giatien." đồng/buổi"; ?> <br/>
                        <?php echo "Đánh giá: ".$gs_khac->gs_kinhnghiem; ?>
                    </div>
                </div>
                <?php
            } ?>
            ...
            foreach ($gs_khac as $gs_khac) { 
                ...
                <div class="panel-body">
                    ...
                    <div class="col-md-8">
                        <a href="<?php echo base_url(); ?>giasu/detail/<?php echo $gs_khac->gs_tk_id; ?>"><b><?php echo $gs_khac->gs_hoten; ?></a></b> <br/>
                        <?php echo "Trình độ: ".$gs_khac->gs_trinhdo; ?> <br/>
                        <?php echo "K.nghiệm: ".$gs_khac->gs_kinhnghiem." năm"; ?>
                    </div>
                </div>
                <?php
            } ?>
